...
Step 1 — Search: POST /search-query with a fresh CSRF token → 50 HTML cards
           (name, reference, price, image, detail URL).
Step 2 — Detail: GET each residence page and parse everything available:
           rooms (type, rent, surface, availability, floor, deposit, fees, energy),
           description, services/amenities, photo gallery, phone, POI coordinates.
Step 3 — Save: write all raw data to raw/nexity_studea.json.
"""
...
RAW_PATH = Path(__file__).parent / "raw" / "nexity_studea.json"

DETAIL_DELAY = 3.0  # seconds — required to avoid CloudFront 403
...
def _table_to_raw(table: Tag) -> dict:
    """Convert a <table> to a dict with headers list and rows list-of-lists."""
    all_rows = table.find_all("tr")
    if not all_rows:
        return {"headers": [], "rows": []}

    # First row — use <th> if present, otherwise first <td> row as header
    first = all_rows[0]
...
def _parse_rooms(soup: BeautifulSoup) -> list[dict]:
    """
    Nexity uses ONE table per group of rooms.  Actual columns (detected via headers):
      Superficie | Étage | Disponibilité | Loyer CC/mois | Frais de dossier
      | Dépôt de garantie | Performance énergétique | Favoris | [Type?]

    Column order varies by page, so we detect each column by keyword.
    The raw table dict is also stored at the top level (room_tables_raw) so the
    normaliser can re-parse if needed.
    """
    ...
    for table in tables:
        ...
        # Only process tables that look like room listings
        is_room_table = any(k in header_text for k in ["loyer", "superficie", "disponib"])
        if not is_room_table:
            continue

        h = raw["headers"]
        c_type = _col(h, "type")
        c_surface = _col(h, "superficie", "surface")
        c_floor = _col(h, "tage")   # "Étage" — accent may vary
        c_dispo = _col(h, "dispo")
        c_rent = _col(h, "loyer", "cc/mois")
        c_fee = _col(h, "frais")
        c_deposit = _col(h, "pot de garantie", "garantie")
        c_energy = _col(h, "nergi")   # "énergétique"
        ...
        for cells in raw["rows"]:
            ...
            rooms.append({
                ...
                "_raw_headers": raw["headers"],  # kept for normaliser debugging
            })
...
def _parse_services(soup: BeautifulSoup) -> list[str]:
    """Extract service/amenity names from the services section."""
    services: list[str] = []

    # Strategy 1: img alt attributes within service containers (name appears as alt)
    ...
    # Strategy 2: deduplicate repeated short strings (name appears twice in text block)
    for section in soup.find_all(class_=re.compile(r"service", re.I)):
...
def _parse_images(html_text: str) -> list[str]:
    """All background-image URLs from media.nexity-studea.com (the gallery)."""
...
def _parse_poi_centroid(html_text: str) -> tuple[float | None, float | None]:
    """Compute mean of all POI lat/lng from the embedded poisJson JS variable."""
...
def _parse_address(soup: BeautifulSoup) -> str | None:
    """Try to find a visible address string on the detail page."""
    for sel in [
...
def _scrape_detail(session: requests.Session, url: str) -> dict:
    """Fetch a detail page and return all parsed fields."""
    try:
...
def fetch() -> list[dict]:
    """Scrape all Nexity Studéa Paris residences and save raw data."""
    session = _session()

    print("[nexity-studea] Fetching CSRF token...")
    token = _csrf_token(session)

    print("[nexity-studea] Searching Paris residences...")
    cards = _search_cards(session, token)
    print(f"[nexity-studea] {len(cards)} cards found")
    ...
    print(f"[nexity-studea] Raw data saved to {RAW_PATH}")
    return results
