...
DETAIL_DELAY = 0.5
RAW_PATH = Path(__file__).parent / "debug" / "studyrama.json"
...
def fetch() -> list[dict]:
    ...
    print("[studyrama] Stage 1: fetching list API...")
    resp = session.get(LIST_URL, headers=LIST_HEADERS, allow_redirects=True, timeout=30)
    resp.raise_for_status()
    items: list[dict] = resp.json()
    print(f"[studyrama] {len(items)} residences found")

    scraped_at = datetime.now(timezone.utc).isoformat()
    enriched = []

    print("[studyrama] Stage 2: scraping detail pages...")
    for item in tqdm(items, desc="studyrama details", unit="page"):
    ...
    print(f"[studyrama] Saved {len(enriched)} records -> {RAW_PATH}")
    return enriched
